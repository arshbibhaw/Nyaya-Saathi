...
import { INDIAN_STATES, UNION_TERRITORIES, ALL_INDIAN_LOCATIONS } from "@/lib/constants/states";
...
export default function NewCasePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    let fullDescription = formData.issue.trim();
    const contextDetails: string[] = [];
    if (formData.amount.trim()) {
      const formattedAmount = formData.amount.startsWith("₹") || formData.amount.toLowerCase().startsWith("rs")
        ? formData.amount
        : `₹${formData.amount}`;
      contextDetails.push(`Amount involved: ${formattedAmount}`);
    }
    if (formData.date) {
      contextDetails.push(`Incident Date: ${formData.date}`);
    }
    if (formData.reported) {
      contextDetails.push(`Previously Reported: ${formData.reported}`);
    }
    if (contextDetails.length > 0) {
      fullDescription = `${fullDescription}\n\n[Context: ${contextDetails.join(" | ")}]`;
    }

    try {
      const newCase = await createCase(fullDescription, selectedLocation);

      // Upload any evidence files
      if (formData.evidence.length > 0) {
        for (const file of formData.evidence) {
          try {
            await api.uploadEvidence(newCase.id, file);
          } catch (uploadErr) {
            console.error("Evidence upload error during creation:", uploadErr);
          }
        }
      }
  ...
  const selectedLocation = ALL_INDIAN_LOCATIONS.find((loc) => loc.value === formData.state)?.label || formData.state || "Not specified";
          ...
          >
            ...
            {step === 2 && (
              <div className="space-y-6">
                ...
                <div className="space-y-2">
                  <Label htmlFor="state">Which State or Union Territory are you in?</Label>
                  <select
                    id="state"
            ...
            {step === 3 && (
              <div className="space-y-4">
                <div className="relative flex flex-col items-center justify-center rounded-lg border-2 border-dashed border-border bg-muted/50 p-8 text-center transition-colors hover:bg-muted/80">
                  <input
                    type="file"
                    ...
                    onChange={(e) => {
                      if (e.target.files && e.target.files.length > 0) {
                        setFormData({
                          ...formData,
                          evidence: [...formData.evidence, ...Array.from(e.target.files)],
                        });
                        e.target.value = "";
                      }
                    }}
                  ...
                  <h3 className="text-sm font-semibold text-foreground">Upload Evidence Documents</h3>
                  <p className="mt-1 text-xs text-muted-foreground">PDF, JPG, PNG, WebP up to 10MB each</p>
                  <Button
                    type="button"
                    variant="outline"
                ...
                {formData.evidence.length > 0 && (
                  <div className="space-y-2 mt-4 max-h-[300px] overflow-y-auto pr-1">
                    <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">
                      Selected Files ({formData.evidence.length})
                    </p>
                    {formData.evidence.map((file, i) => (
                      <div key={i} className="flex items-center gap-3 rounded-md border p-3 bg-card shadow-sm">
                  ...
                  <CardContent className="p-6">
                    ...
                    <dl className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                      <div>
                        ...
                        <dd className="mt-1 text-sm text-foreground line-clamp-2">{formData.issue || "Legal Query / Case"}</dd>
                      </div>
                      <div>
                        <dt className="text-sm font-medium text-muted-foreground">Location</dt>
                        <dd className="mt-1 text-sm text-foreground font-medium">{selectedLocation}</dd>
                      </div>
                      <div>
                        ...
                        <dd className="mt-1 text-sm text-foreground font-medium">
                          {formData.evidence.length === 0
                            ? "No files attached"
                            : `${formData.evidence.length} ${formData.evidence.length === 1 ? "file" : "files"}`}
                        </dd>
                      </div>
                      <div>
